...
def buscar_piso_por_ordenante_en_historico(excel_registros, mov: Dict) -> str | None:
    concepto = mov["concepto_original"].upper()
    nombres = extraer_nombres_desde_concepto(concepto)
    if len(nombres) < 2:
        return None
...
def aplicar_busqueda_por_ordenante(excel_registros, movimientos: List[Dict]) -> List[Dict]:
    for mov in movimientos:
        if mov.get("piso"):
            continue
        piso = buscar_piso_por_ordenante_en_historico(excel_registros, mov)
        if piso:
            mov["piso"] = piso
    return movimientos
